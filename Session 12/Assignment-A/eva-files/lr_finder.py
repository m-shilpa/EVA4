    ...
    def range_test(
        ...
        Arguments:
            ...
            accumulation_steps (int, optional): steps for gradient accumulation. If it
                ...
                Gradient accumulation - accumulate the gradient for each mini-batch without updating the weights for a certain no. of steps.
                Then sum the accumulated gradients and divide by no. of steps to get the final gradient.
        ...
        Gradient accumulation is supported; example:
            ...
            >>> desired_bs, real_bs = 32, 4         # batch size, desired_bs is the mini-batches of the real_bs,4 split into 32
        ...
        for iteration in tqdm(range(num_iter)): # tqdm produces progress bar for the for loop
            ...
            # Check if the loss has diverged(if loss crossed the highest threshold); if it has, stop the test
...
class LinearLR(_LRScheduler):
    ...
    """ The lr increases by a factor of r for each iteration.
        r changes for each iteration.
        base_lrs contains [start_lr] and is fixed. 
    """
    def get_lr(self):
